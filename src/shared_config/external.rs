...
impl ExternalServiceConfig {
    ...
    pub fn runner_url(&self) -> Option<Url> {
        self.runner_url.as_ref().and_then(|s| {
            Url::parse(s)
                .map_err(|e| {
                    tracing::error!("Invalid runner URL '{}': {}", s, e);
                    e
                })
                .ok()
        })
    ...
    pub fn differ_url(&self) -> Option<Url> {
        self.differ_url.as_ref().and_then(|s| {
            Url::parse(s)
                .map_err(|e| {
                    tracing::error!("Invalid differ URL '{}': {}", s, e);
                    e
                })
                .ok()
        })
    ...
    pub fn publisher_url(&self) -> Option<Url> {
        self.publisher_url.as_ref().and_then(|s| {
            Url::parse(s)
                .map_err(|e| {
                    tracing::error!("Invalid publisher URL '{}': {}", s, e);
                    e
                })
                .ok()
        })
    ...
    pub fn archiver_url(&self) -> Option<Url> {
        self.archiver_url.as_ref().and_then(|s| {
            Url::parse(s)
                .map_err(|e| {
                    tracing::error!("Invalid archiver URL '{}': {}", s, e);
                    e
                })
                .ok()
        })
    ...
    pub fn git_store_url(&self) -> Option<Url> {
        self.git_store_url.as_ref().and_then(|s| {
            Url::parse(s)
                .map_err(|e| {
                    tracing::error!("Invalid git store URL '{}': {}", s, e);
                    e
                })
                .ok()
        })
    ...
    pub fn bzr_store_url(&self) -> Option<Url> {
        self.bzr_store_url.as_ref().and_then(|s| {
            Url::parse(s)
                .map_err(|e| {
                    tracing::error!("Invalid bzr store URL '{}': {}", s, e);
                    e
                })
                .ok()
        })
