...
use janitor_runner::{AppState, database::RunnerDatabase};
...
async fn test_ready_endpoint_compatibility() {
    ...
    assert!(matches!(response.status(), StatusCode::OK | StatusCode::SERVICE_UNAVAILABLE));
...
async fn test_metrics_endpoint_compatibility() {
    ...
    assert!(metrics_text.contains("# HELP") || metrics_text.contains("# TYPE") || metrics_text.is_empty());
}
...
async fn test_run_endpoints_compatibility() {
    ...
    assert!(matches!(response.status(), StatusCode::OK | StatusCode::BAD_REQUEST | StatusCode::NOT_FOUND));
...
async fn test_http_status_codes_compatibility() {
    ...
    let test_cases = vec![
        ("/health", Method::GET, vec![StatusCode::OK, StatusCode::SERVICE_UNAVAILABLE]),
        ("/ready", Method::GET, vec![StatusCode::OK, StatusCode::SERVICE_UNAVAILABLE]),
        ...
        ("/queue/next", Method::GET, vec![StatusCode::OK, StatusCode::NOT_FOUND]),
    ];
