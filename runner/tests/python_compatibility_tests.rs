...
use janitor_runner::{committer_env, is_log_filename, QueueItem, JanitorResult, VcsInfo};
...
fn test_is_log_filename_compatibility() {
    ...
    for filename in valid_log_files {
        assert!(is_log_filename(filename), "Expected {} to be a log filename", filename);
    }
    ...
    for filename in invalid_log_files {
        assert!(!is_log_filename(filename), "Expected {} to NOT be a log filename", filename);
    }
}
...
fn test_queue_item_json_compatibility() {
    ...
    assert_eq!(queue_item.estimated_duration, Some(Duration::from_secs(300)));
...
fn test_janitor_result_compatibility() {
    ...
    assert_eq!(json_value["description"], "Successfully applied lintian fixes");
    ...
    assert_eq!(deserialized.description, Some("No changes needed".to_string()));
...
fn test_database_row_compatibility() {
    ...
    row_data.insert("id".to_string(), Value::Number(serde_json::Number::from(12345)));
    row_data.insert("context".to_string(), json!({"branch": "main"}));
    row_data.insert("command".to_string(), Value::String("lintian-fixes".to_string()));
    row_data.insert("estimated_duration".to_string(), Value::Number(serde_json::Number::from(300)));
    row_data.insert("campaign".to_string(), Value::String("lintian-fixes".to_string()));
    row_data.insert("refresh".to_string(), Value::Bool(false));
    row_data.insert("requester".to_string(), Value::Null);
    row_data.insert("change_set".to_string(), Value::String("cs-123".to_string()));
    row_data.insert("codebase".to_string(), Value::String("example-package".to_string()));
...
fn test_campaign_config_compatibility() {
    ...
    for campaign in valid_campaigns {
        ...
        assert!(campaign.chars().all(|c| c.is_ascii_lowercase() || c.is_ascii_digit() || "+-".contains(c)));
    }
...
fn test_error_handling_compatibility() {
    ...
    let python_exceptions = HashMap::from([
        ...
        ("QueueItemAlreadyClaimed", "Queue item already assigned to worker"),
        ("QueueRateLimiting", "Rate limit exceeded"),
        ("BranchOpenFailure", "Failed to open branch"),
        ("UnsupportedVcs", "VCS type not supported"),
        ("CandidateUnavailable", "Candidate not available for processing"),
    ]);
