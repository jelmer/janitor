...
use janitor_runner::{
    ...
    config::{RunnerConfig, DatabaseConfig, WebConfig, ApplicationConfig},
};
use serde_json::json;
...
async fn test_application_lifecycle() {
    ...
    let app = Application::builder_from_config(config)
        .build()
        .await;
...
async fn test_configuration_loading() {
    ...
    assert_eq!(default_config.database.url, "postgresql://localhost/janitor");
...
async fn test_error_tracking() {
    ...
    use janitor_runner::error_tracking::{ErrorTracker, ErrorTrackingConfig, TrackedError, ErrorSeverity, ErrorCategory};
...
async fn test_performance_monitoring() {
    ...
    use janitor_runner::performance::{PerformanceMonitor, PerformanceConfig};
...
async fn test_log_manager() {
    ...
    use janitor_runner::logs::{LogFileManager, LogConfig, LogStorageBackend};
...
async fn test_artifact_manager() {
    ...
    use janitor_runner::artifacts::{ArtifactManager, ArtifactConfig, ArtifactStorageBackend};
    use std::path::PathBuf;
    use std::collections::HashMap;
    ...
    // Store artifact
    let store_result = manager.store_artifact(
        run_id, 
        name, 
        test_content, 
        "text/plain",
    ...
    match store_result {
        ...
        Err(e) => {
            println!("Artifact storage failed (may be expected in test env): {}", e);
        }
    }
}
...
async fn test_system_integration() {
    ...
    let config = test_config();
    let app_result = Application::builder_from_config(config)
        .build()
        .await;
