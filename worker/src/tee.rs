...
impl CopyOutput {
    ...
    pub fn new(output_log: &std::path::Path, tee: bool) -> io::Result<Self> {
        ...
        let old_stdout = dup(nix::libc::STDOUT_FILENO)
            .map_err(|e| io::Error::new(io::ErrorKind::Other, format!("Failed to duplicate stdout: {}", e)))?;
            
        let old_stderr = dup(nix::libc::STDERR_FILENO)
            .map_err(|e| {
    ...
    fn setup_tee_redirection(&mut self, output_log: &std::path::Path) -> io::Result<()> {
        ...
        let stdin_fd = process.stdin.as_ref()
            .ok_or_else(|| io::Error::new(io::ErrorKind::Other, "Tee process has no stdin"))?
            .as_raw_fd();

        // Redirect stdout and stderr to tee's stdin
        dup2(stdin_fd, nix::libc::STDOUT_FILENO)
            .map_err(|e| io::Error::new(io::ErrorKind::Other, format!("Failed to redirect stdout to tee: {}", e)))?;
            
        dup2(stdin_fd, nix::libc::STDERR_FILENO)
            .map_err(|e| io::Error::new(io::ErrorKind::Other, format!("Failed to redirect stderr to tee: {}", e)))?;
    ...
    fn setup_file_redirection(&mut self, output_log: &std::path::Path) -> io::Result<()> {
        ...
        dup2(file_fd, nix::libc::STDOUT_FILENO)
            .map_err(|e| io::Error::new(io::ErrorKind::Other, format!("Failed to redirect stdout to file: {}", e)))?;
            
        dup2(file_fd, nix::libc::STDERR_FILENO)
            .map_err(|e| io::Error::new(io::ErrorKind::Other, format!("Failed to redirect stderr to file: {}", e)))?;
    ...
    pub fn restore(&mut self) -> io::Result<()> {
        if let (Some(old_stdout), Some(old_stderr)) = (self.old_stdout.take(), self.old_stderr.take()) {
            // Restore original stdout and stderr
            dup2(old_stdout, nix::libc::STDOUT_FILENO)
                .map_err(|e| io::Error::new(io::ErrorKind::Other, format!("Failed to restore stdout: {}", e)))?;
                
            dup2(old_stderr, nix::libc::STDERR_FILENO)
                .map_err(|e| io::Error::new(io::ErrorKind::Other, format!("Failed to restore stderr: {}", e)))?;

            // Close the duplicated file descriptors
            close(old_stdout)
                .map_err(|e| io::Error::new(io::ErrorKind::Other, format!("Failed to close old stdout: {}", e)))?;
                
            close(old_stderr)
                .map_err(|e| io::Error::new(io::ErrorKind::Other, format!("Failed to close old stderr: {}", e)))?;
        }
        ...
        if self.tee {
            if let Some(ref mut process) = self.process.take() {
                process.wait()
                    .map_err(|e| io::Error::new(io::ErrorKind::Other, format!("Failed to wait on tee process: {}", e)))?;
            }
        } else if let Some(ref mut file) = self.newfd.take() {
            file.flush()
                .map_err(|e| io::Error::new(io::ErrorKind::Other, format!("Failed to flush output file: {}", e)))?;
        }
...
impl Drop for CopyOutput {
    fn drop(&mut self) {
        ...
        if let Err(e) = self.restore() {
            eprintln!("Warning: Failed to restore file descriptors during drop: {}", e);
