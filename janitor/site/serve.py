...
class ForwardedResource(PrefixResource):
    ...
    def get_info(self):
        return {"location": self._location, "prefix": self._prefix}
    ...
    async def resolve(self, request):
        ...
        match_dict = {
            "path": URL.build(path=path[len(self._prefix) + 1 :], encoded=True).path
        }
...
""", session_id, userinfo)
    ...
    app.router.register_resource(
        ForwardedResource("bzr", args.vcs_store_url.rstrip("/") + "/bzr")
    )
    app.router.register_resource(
