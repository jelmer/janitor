...
def report_failure(kind, description, original):
    ...
    if "DIST_RESULT" in os.environ:
        ...
        with open(os.environ["DIST_RESULT"], "w") as f:
            json.dump({"result_code": kind, "description": description}, f)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--schroot", type=str, default=os.environ.get("SCHROOT"), help="Schroot to use"
    )
    parser.add_argument(
        "--target-dir", type=str, default="..", help="Directory to write to"
    )
    parser.add_argument(
        "--packaging", type=str, default=None, help="Location of packaging"
    )
    parser.add_argument(
        "--directory",
        "-d",
    ...
    parser.add_argument("--apt-update", action="store_true")
    parser.add_argument("--apt-dist-upgrade", action="store_true")
    ...
    with ExitStack() as es:
        ...
        try:
            ...
            if e.errno == errno.ENOSPC:
                report_failure(
                    "no-space-on-device", "No space on device running mkdtemp", e
                )
                sys.exit(1)
            raise
        ...
        if args.packaging:
            (packaging_tree, packaging_debian_path) = WorkingTree.open_containing(
                args.packaging
            )
            ...
            try:
                ...
                logging.warning(
                    "Ignoring error installing declared build dependencies " "(%s): %s",
                    e.error.kind,
                    str(e.error),
                )
        ...
        try:
            ...
            if args.log_directory:
                log_manager = DirectoryLogManager(
                    os.path.join(args.log_directory, DIST_LOG_FILENAME), mode="redirect"
                )
            else:
