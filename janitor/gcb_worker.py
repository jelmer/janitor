...
async def run_gcb_worker(logf, output_directory, args, timeout=None):
    ...
    if ops["status"] == "SUCCESS":
        ...
        download_results(http, bearer, artifact_manifest_url, output_directory)
        tgz_name = os.environ["PACKAGE"] + ".tgz"
        tgz_path = os.path.join(output_directory, tgz_name)
        if os.path.exists(tgz_path):
            subprocess.check_call(["tar", "xfz", tgz_name], cwd=output_directory)
            os.unlink(tgz_path)
    else:
...
def main(argv=None):
    ...
    parser.add_argument(
        "--timeout", type=int, help="Build timeout (in seconds)", default=3600
    )
    args, unknown = parser.parse_known_args()

    asyncio.run(
        run_gcb_worker(sys.stdout.buffer, args.output_directory, unknown, args.timeout)
    )
    return 0
