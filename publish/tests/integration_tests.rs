...
use janitor_publish::{calculate_next_try_time, AppState, PublishWorker, PublishError, CheckMpError};
...
mod signature_compatibility_tests {
    ...
    fn test_queue_processing_functions_exist() {
        ...
        let _publish_pending_ready: fn(
            std::sync::Arc<AppState>,
            Option<usize>,
            bool,
        ) -> _ = publish_pending_ready;
        
        let _refresh_bucket_mp_counts: fn(
            std::sync::Arc<AppState>,
        ) -> _ = refresh_bucket_mp_counts;
        
        let _listen_to_runner: fn(
            std::sync::Arc<AppState>,
