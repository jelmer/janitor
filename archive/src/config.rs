...
impl AptRepositoryConfig {
    ...
    pub fn component_arch_path(&self, component: &str, arch: &str) -> PathBuf {
        self.suite_path().join(component).join(format!("binary-{}", arch))
    }
    ...
    pub fn validate(&self) -> ArchiveResult<()> {
        if self.name.is_empty() {
            return Err(ArchiveError::InvalidConfiguration("Repository name cannot be empty".to_string()));
        }
        
        if self.suite.is_empty() {
            return Err(ArchiveError::InvalidConfiguration("Suite name cannot be empty".to_string()));
        }
        
        if self.architectures.is_empty() {
            return Err(ArchiveError::InvalidConfiguration("At least one architecture must be specified".to_string()));
        }
        
        if self.components.is_empty() {
            return Err(ArchiveError::InvalidConfiguration("At least one component must be specified".to_string()));
        }
...
impl ArchiveConfig {
    ...
    pub fn from_file(path: &std::path::Path) -> ArchiveResult<Self> {
        let content = std::fs::read_to_string(path)
            .map_err(|e| ArchiveError::Configuration(format!("Failed to read config file: {}", e)))?;
    ...
    pub fn to_file(&self, path: &std::path::Path) -> ArchiveResult<()> {
        let content = serde_json::to_string_pretty(self)
            .map_err(|e| ArchiveError::Configuration(format!("Failed to serialize config: {}", e)))?;
        
        std::fs::write(path, content)
            .map_err(|e| ArchiveError::Configuration(format!("Failed to write config file: {}", e)))?;
    ...
    pub fn validate(&self) -> ArchiveResult<()> {
        for (name, repo) in &self.repositories {
            repo.validate()
                .map_err(|e| ArchiveError::InvalidConfiguration(format!("Repository '{}': {}", name, e)))?;
        }
        
        if let Some(gpg) = &self.gpg {
            if gpg.key_id.is_empty() {
                return Err(ArchiveError::InvalidConfiguration("GPG key ID cannot be empty".to_string()));
            }
        }
