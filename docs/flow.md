...
Once the janitor knows about a codebase, candidates can be created. A candidate
is a bit of data that a particular campaign (e.g. lintian-fixes)
can be run on a particular codebase and that there is some chance it will yield
changes.
...
simple, in which case they just output a candidate for each codebase in a campaign
with fixed settings for value and success_chance.
 ...
 * previous success rate (for the campaign/codebase combination and the codebase itself)
