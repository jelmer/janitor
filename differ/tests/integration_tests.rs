...
impl janitor::artifacts::ArtifactManager for MockArtifactManager {
    async fn retrieve_artifacts(
        ...
        _filter: Option<&(dyn for<'a> Fn(&'a str) -> bool + Sync + Send)>,
    ...
    async fn get_artifact(
        ...
        Ok(Box::new(std::io::Cursor::new(
            b"mock artifact content".to_vec(),
        )))
    }

    fn public_artifact_url(&self, run_id: &str, filename: &str) -> url::Url {
        url::Url::parse(&format!("http://mock.artifacts/{}/{}", run_id, filename)).unwrap()
    }

    async fn iter_ids(&self) -> Box<dyn Iterator<Item = String> + Send> {
        Box::new(vec!["test1".to_string(), "test2".to_string()].into_iter())
    }
...
fn create_test_app_state(
    has_artifacts: bool,
    db: sqlx::PgPool,
) -> Arc<janitor_differ::test_utils::TestAppState> {
    let artifact_manager = Box::new(MockArtifactManager::new(has_artifacts));
    Arc::new(janitor_differ::test_utils::TestAppState::new(
        db,
        artifact_manager,
    ))
}
...
async fn test_cache_path_creation() {
    ...
    assert_eq!(
        expected_diffoscope_file.file_name().unwrap(),
        "old123_new456.json"
    );
...
async fn test_diffoscope_filtering() {
    let mut diff: janitor_differ::diffoscope::DiffoscopeOutput = serde_json::from_value(json!({
        "diffoscope-json-version": 1,
        "source1": "/full/path/to/old_package_1.0.deb",
        "source2": "/full/path/to/new_package_1.1.deb",
        "comments": [],
        "unified_diff": null,
        "details": []
    }))
