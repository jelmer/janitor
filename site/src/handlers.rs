...
pub async fn index(State(state): State<AppState>) -> AxumResult<Html<String>> {
    ...
    match state.database.get_stats().await {
        Ok(stats) => {
            context.insert("total_packages", &stats.get("total_codebases").unwrap_or(&0));
            context.insert("active_runs", &stats.get("active_runs").unwrap_or(&0));
            context.insert("queue_size", &stats.get("queue_size").unwrap_or(&0));
            context.insert("recent_successful_runs", &stats.get("recent_successful_runs").unwrap_or(&0));
        }
        Err(e) => {
...
pub async fn package_list(
    ...
    match state.database.get_codebases(Some(per_page), Some(offset), query.search.as_deref()).await {
        Ok(packages) => {
            context.insert("packages", &packages);
            
            // Get total count for pagination
            match state.database.count_codebases(query.search.as_deref()).await {
                Ok(total_count) => {
...
pub async fn package_detail(
    ...
    let runs = match state.database.get_runs_for_codebase(&package_name, Some(20), Some(0)).await {
        Ok(runs) => runs,
        Err(e) => {
