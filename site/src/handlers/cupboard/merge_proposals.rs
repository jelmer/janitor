...
pub async fn mp_dashboard(
    ...
    match fetch_mp_dashboard_data(&state, &filters).await {
        ...
        Err(e) => {
            ...
            context.insert(
                "error_message",
                &format!("Failed to load merge proposal data: {}", e),
            );
        }
    }
    ...
    match content_type {
        ...
        _ => {
            match state
                .templates
                .render("cupboard/merge-proposals.html", &context)
            {
...
pub async fn mp_details(
    ...
    match content_type {
        ...
        _ => {
            match state
                .templates
                .render("cupboard/merge-proposal.html", &context)
            {
                Ok(html) => Html(html).into_response(),
                Err(e) => {
                    tracing::error!("Template rendering error: {}", e);
...
pub async fn forge_health(
    ...
    match fetch_forge_health(&state).await {
        ...
        Err(e) => {
            ...
            context.insert(
                "error_message",
                &format!("Failed to load forge health data: {}", e),
            );
        }
    }
    ...
    match content_type {
        ...
        _ => {
            match state
                .templates
                .render("cupboard/forge-health.html", &context)
            {
                Ok(html) => Html(html).into_response(),
                Err(e) => {
                    tracing::error!("Template rendering error: {}", e);
...
async fn fetch_mp_dashboard_data(
    ...
    let merge_proposals = vec![
        MergeProposal {
            ...
            description: Some(
                "This merge request fixes multiple lintian issues found in the package".to_string(),
            ),
            merged_at: None,
            merged_by: None,
...
async fn fetch_mp_details(
    ...
    let publish_history = vec![MpPublishHistory {
...
async fn fetch_forge_health(state: &AppState) -> anyhow::Result<HashMap<String, ForgeHealth>> {
