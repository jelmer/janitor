...
impl OidcClient {
    ...
    pub async fn new(config: &SiteConfig) -> Result<Self, AuthError> {
        ...
        let external_url = config.external_url.as_ref()
    ...
    async fn discover_config(http_client: &HttpClient, issuer_url: &str) -> Result<OidcConfig, AuthError> {
    ...
    fn parse_user_info(&self, userinfo: serde_json::Value) -> Result<User, AuthError> {
        ...
        let preferred_username = userinfo.get("preferred_username")
            .and_then(|v| v.as_str())
            .map(|s| s.to_string());
