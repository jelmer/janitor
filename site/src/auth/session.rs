...
impl SessionManager {
    ...
    pub async fn create_session(&self, user: User) -> Result<String, SessionError> {
        ...
        let userinfo_json = serde_json::to_value(&session_info)
            .map_err(|e| SessionError::InvalidData(format!("Failed to serialize session: {}", e)))?;
        
        sqlx::query(
            "INSERT INTO site_session (id, timestamp, userinfo) VALUES ($1, $2, $3)"
        )
        .bind(&session_id)
    ...
    pub async fn get_session(&self, session_id: &str) -> Result<SessionInfo, SessionError> {
        ...
        let session_info: SessionInfo = serde_json::from_value(userinfo)
            .map_err(|e| SessionError::InvalidData(format!("Failed to deserialize session: {}", e)))?;
    ...
    pub async fn update_activity(&self, session_id: &str) -> Result<(), SessionError> {
        let result = sqlx::query(
            "UPDATE site_session SET timestamp = $1 WHERE id = $2"
        )
        .bind(Utc::now())
        .bind(session_id)
        .execute(&self.pool)
    ...
    pub async fn delete_session(&self, session_id: &str) -> Result<(), SessionError> {
        sqlx::query(
            "DELETE FROM site_session WHERE id = $1"
        )
        .bind(session_id)
        .execute(&self.pool)
        .await?;
    ...
    pub async fn cleanup_expired_sessions(&self) -> Result<u64, SessionError> {
        ...
        let result = sqlx::query(
            "DELETE FROM site_session WHERE timestamp < $1"
        )
        .bind(cutoff)
        .execute(&self.pool)
        .await?;
