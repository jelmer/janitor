    ...
    S: Send + Sync,
{
    ...
    async fn from_request_parts(parts: &mut Parts, _state: &S) -> Result<Self, Self::Rejection> {
        let user = parts.extensions.get::<UserContext>().cloned();
        Ok(OptionalUser(user))
    }
}

/// Middleware function to require login
pub async fn require_login(req: Request, next: Next) -> Result<Response, StatusCode> {
...
pub async fn require_qa_reviewer(req: Request, next: Next) -> Result<Response, StatusCode> {
    let (parts, body) = req.into_parts();
...
impl AuthState {
    pub fn new(session_manager: SessionManager, config: &SiteConfig) -> Self {
...
) -> axum::middleware::FromFnLayer<impl Clone, Arc<AuthState>, ()> {
