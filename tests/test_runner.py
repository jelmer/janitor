...
async def test_submit_candidate(aiohttp_client, db, tmp_path):
    ...
    # example.com doesn't actually serve foo.git, so the branch never opens
    # and vcs_type is never resolved for this codebase (no cached db value
    # either). branch.vcs_type still falls back to DEFAULT_VCS_TYPE since
    # it's a required field worker-side (Branch.vcs_type: VcsType), but
    # target_repository.vcs_type is left honestly unresolved rather than
    # assuming git, and there is no cached branch to report since we don't
    # know which vcs_type's store to look in.
...
async def test_assignment_no_vcs_manager_configured(aiohttp_client, db):
    # No git/bzr managers configured at all - a deployment missing
    # git_location entirely. vcs_type is never resolved for this codebase
    # (no branch_url, no cached db value), so DEFAULT_VCS_TYPE ("git") is
    # not even a manager this deployment has. public_vcs_managers[DEFAULT_VCS_TYPE]
    # used to KeyError uncaught here. target_repository.url is a required
    # field worker-side (Url, not Option<Url>), so there is nothing left to
    # fall back to - the run gets aborted with a clear reason instead of
    # shipping an assignment the worker can't deserialize.
