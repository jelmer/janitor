    ...
    async fn ensure_smart_server(&self, repo_path: &std::path::Path) -> Result<()> {
        ...
        if server_guard.is_none() {
            Python::with_gil(|py| {
                ...
                let smart_module = py.import_bound("breezy.bzr.smart")
                    .map_err(|e| BzrError::Python(format!("Failed to import smart module: {}", e)))?;
                
                let server_module = py.import_bound("breezy.bzr.smart.server")
                ...
                *server_guard = Some(smart_server.unbind());
                info!("Initialized Bazaar smart server for: {}", repo_path.display());
                Ok::<(), BzrError>(())
            })?
        }
        ...
        let response_bytes = Python::with_gil(|py| -> Result<Vec<u8>> {
            ...
            match smart_server.call_method1("process_request", (py_request,)) {
                Ok(response) => {
                    ...
                    let response_bytes = response.extract::<Vec<u8>>()
                        .map_err(|e| BzrError::Python(format!("Failed to extract response: {}", e)))?;
                    
                    Ok(response_bytes)
                }
                Err(e) => {
                    error!("Smart protocol error: {}", e);
                    Err(BzrError::Python(format!("Smart protocol processing failed: {}", e)))
                }
            }
        })?;
...
pub async fn smart_protocol_handler(
    ...
    info!("Smart protocol request for {}/{}/{}", campaign, codebase, role);
...
pub async fn smart_protocol_subprocess_handler(
    ...
    let mut child = Command::new("brz")
        .args(["serve", "--inet", "--directory", &repo_path.to_string_lossy()])
    ...
    if let Some(mut stdout) = child.stdout.take() {
        stdout.read_to_end(&mut response_bytes).await
            .map_err(|e| BzrError::Io(e))?;
    }
    ...
    debug!("Smart protocol subprocess response: {} bytes", response_bytes.len());
...
pub async fn serve_bzr_file_handler(
    ...
    let repo_base = state.config.repository_path
        .join(&repo_path.campaign)
        .join(&repo_path.codebase)
        .join(&repo_path.role);
    
    let fs_path = fs_path.canonicalize()
        .map_err(|_| BzrError::PathNotFound { path: file_path.clone() })?;
