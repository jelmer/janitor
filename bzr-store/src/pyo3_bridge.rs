...
impl BreezyOperations {
    ...
    pub async fn get_diff(repo_path: &Path, old_revid: &str, new_revid: &str) -> Result<Vec<u8>> {
        ...
        Python::with_gil(|py| {
            ...
            let old_tree = repo.call_method1("revision_tree", (old_revid,))
                .map_err(|e| BzrError::Python(format!("Failed to get old tree: {}", e)))?;
            
            let new_tree = repo.call_method1("revision_tree", (new_revid,))
                .map_err(|e| BzrError::Python(format!("Failed to get new tree: {}", e)))?;
            
            // Import diff module
            let diff_module = py.import_bound("breezy.diff")
            ...
            let diff_output = io_module.call_method0("StringIO")
                .map_err(|e| BzrError::Python(format!("Failed to create StringIO: {}", e)))?;
            
            // Generate unified diff - clone diff_output to avoid move
            diff_module.call_method("show_diff_trees", (old_tree, new_tree, diff_output.clone()), None)
                .map_err(|e| BzrError::Python(format!("Failed to generate diff: {}", e)))?;
            
            // Get the diff content
            let diff_content = diff_output.call_method0("getvalue")
    ...
    pub async fn get_revision_info(repo_path: &Path, old_revid: &str, new_revid: &str) -> Result<Vec<BreezyRevisionInfo>> {
        ensure_python_initialized();
        
        Python::with_gil(|py| {
            debug!("Getting revision info for repository: {}", repo_path.display());
            ...
            // Get revision range
            let revisions = repo.call_method("get_revision_history", (old_revid, new_revid), None)
                    ...
                    Ok(iter) => {
                        for revision_result in iter {
                            match revision_result {
                                Ok(rev) => {
                                    let revision_id = rev.getattr("revision_id")
                                        .and_then(|id| id.extract::<String>())
                                        .unwrap_or_else(|_| "unknown".to_string());
                                    
                                    let message = rev.getattr("message")
                                        .and_then(|msg| msg.extract::<String>())
                                        .unwrap_or_else(|_| "".to_string());
                                    
                                    let committer = rev.getattr("committer")
                                        .and_then(|c| c.extract::<String>())
                                        .unwrap_or_else(|_| "unknown".to_string());
                                    
                                    let timestamp = rev.getattr("timestamp")
                                        .and_then(|ts| ts.extract::<String>())
                                        .unwrap_or_else(|_| "unknown".to_string());
                                    
                                    let parents = rev.getattr("parent_ids")
    ...
    pub async fn configure_remote(repo_path: &Path, remote_url: &str) -> Result<()> {
        ...
        Python::with_gil(|py| {
            ...
            branch_obj.call_method("set_parent", (remote_url,), None)
    ...
    pub async fn get_transport(url: &str) -> Result<String> {
        ...
        Python::with_gil(|py| {
            ...
            let transport = py.import_bound("breezy.transport")
                .map_err(|e| BzrError::Python(format!("Failed to import transport: {}", e)))?;
            
            // Get transport
            let transport_obj = transport.call_method1("get_transport", (url,))
                .map_err(|e| BzrError::Python(format!("Failed to get transport: {}", e)))?;
            
            // Get transport base URL
            let base_url = transport_obj.call_method0("base")
    ...
    pub async fn clone_repository(source_url: &str, target_path: &Path) -> Result<()> {
        ...
        Python::with_gil(|py| {
            debug!("Cloning repository from {} to {}", source_url, target_path.display());
            ...
            // Clone repository
            bzrdir.call_method("clone", (source_url, target_str.as_ref()), None)
                .map_err(|e| BzrError::Python(format!("Failed to clone repository: {}", e)))?;
            
            info!("Successfully cloned repository from {} to {}", source_url, target_path.display());
            Ok(())
        })
    }
    ...
    pub async fn has_uncommitted_changes(repo_path: &Path) -> Result<bool> {
        ...
        Python::with_gil(|py| {
            ...
            let workingtree = py.import_bound("breezy.workingtree")
                .map_err(|e| BzrError::Python(format!("Failed to import workingtree: {}", e)))?;
            
            let path_str = repo_path.to_string_lossy();
            
            // Open working tree
            let tree = workingtree.call_method1("WorkingTree.open", (path_str.as_ref(),))
                .map_err(|e| BzrError::Python(format!("Failed to open working tree: {}", e)))?;
            
            // Check for changes
            let has_changes = tree.call_method0("has_changes")
...
mod tests {
    ...
    async fn test_advanced_operations() {
        ...
        if BreezyOperations::init_shared_repository(&repo_path).await.is_ok() {
            if BreezyOperations::init_branch(&repo_path, None).await.is_ok() {
                ...
                if BreezyOperations::configure_remote(&repo_path, remote_url).await.is_ok() {
