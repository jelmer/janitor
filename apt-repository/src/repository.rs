...
mod tests {
    use super::*;
    use crate::{packages::new_package, sources::new_source};
    ...
    fn test_memory_providers() {
        ...
        packages.add_package(new_package("test-pkg", "1.0.0", "amd64", "test.deb", 1024));

        let mut sources = SourceFile::new();
        sources.add_source(new_source("test-src", "1.0.0", "any", "pool/main/t/test"));
