...
impl RedisClient {
    ...
    async fn ensure_connected(&mut self) -> Result<&mut MultiplexedConnection> {
        ...
        self.connection.as_mut()
            .ok_or_else(|| UploadError::Redis(redis::RedisError::from((
    ...
    where
        F: Fn(BuildResultMessage) -> futures::future::BoxFuture<'static, Result<()>> + Send + Sync + 'static,
    {
