    ...
    fn parse_janitor_config(contents: &str) -> Result<Self> {
        ...
        Ok(Config {
            redis_location: redis_location
                .context("Missing redis_location in config")?,
            database_location: database_location
                .context("Missing database_location in config")?,
            artifact_location: artifact_location
                .context("Missing artifact_location in config")?,
        })
    }
}
...
"#;
        ...
        assert_eq!(config.artifact_location, "file:///var/lib/janitor/artifacts");
    }
}