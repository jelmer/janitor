...
use crate::backfill::{BackfillConfig, BackfillProcessor};
...
impl ServiceOrchestrator {
    ...
    async fn spawn_web_server(&self) -> Result<TaskHandle> {
        let app =
            web::create_app_with_health(self.config.clone(), Some(self.health_status.clone()));
    ...
    async fn run_redis_listener_once(
        ...
        let message_handler =
            match MessageHandler::new(&config.artifact_location, upload_config.clone()).await {
    ...
    async fn run_backfill_mode(&self) -> Result<()> {
        ...
        if summary.failed_uploads > 0 {
            ...
            return Err(crate::error::UploadError::Config(format!(
                "{} uploads failed",
