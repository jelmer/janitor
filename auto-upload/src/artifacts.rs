...
impl ArtifactProcessor {
    ...
    pub async fn new(artifact_location: &str) -> Result<Self> {
        let artifact_manager = get_artifact_manager(artifact_location)
            .await
            .map_err(|e| UploadError::Config(format!("Failed to create artifact manager: {}", e)))?;
    ...
    pub async fn retrieve_artifacts(&self, run_id: &str) -> Result<TempDir> {
        ...
        let temp_dir = TempDir::new()
            .map_err(|e| UploadError::Io(e))?;
        
        match self.artifact_manager.retrieve_artifacts(run_id, temp_dir.path(), None).await {
            Ok(_) => {
            ...
            if in_files_section {
                if line.starts_with(' ') || line.starts_with('\t') {
                    ...
                    if parts.len() >= 5 {
                        ...
                        let file_path = changes_path.parent()
                            .unwrap_or(Path::new("."))
                            .join(filename);
...
"#;
        ...
        let files = ArtifactValidator::find_referenced_files(&changes_path).await.unwrap();
        ...
        assert!(files.iter().any(|f| f.ends_with("test_1.0-1.debian.tar.xz")));
        assert!(files.iter().any(|f| f.ends_with("test_1.0-1_amd64.deb")));
        assert!(files.iter().any(|f| f.ends_with("test_1.0-1_amd64.changes")));
    }
}